...
(
    ...
    const bool gate_first,      // apply the gate before the norm instead of after (Mamba2 style)
    const int gate_act          // 0 = silu (GDN/Mamba2), 1 = sigmoid (KDA, qwen4_exp)
)
{
...
- gate_first: y = norm(x * act(g)) * w instead of norm(x) * w * act(g) (Mamba2 style)
- gate_act: 0 = silu (GDN/Mamba2), 1 = sigmoid (KDA, qwen4_exp's output_gate_type)
*/
