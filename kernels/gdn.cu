...
// Register-resident variant of the 128/128 kernel for the no-history (prefill)
// path. The kernel below keeps the recurrent state in GLOBAL memory and touches
// it three times per step (two reads + one write) — at 48 heads x v_split CTAs x
// seqlen steps that is ~18 GB of state traffic per layer for a 2048-token chunk,
// which at L2 speed is ~3.6 ms and matches the 4.87 ms measured. The FLOP count
// is only ~150 GFLOP, so the kernel was never compute bound; it was streaming
// its own state through L2 2048 times.
//
// With `save_history == false` only the FINAL state is needed, so each thread
// can hold its slice in registers for the whole sequence and write back once.
// Thread (t, bt) owns state[d][v_start + t] for d in [bt*BTS, (bt+1)*BTS) — the
// exact slice the global-memory loops below walk — which is BTS = 128/SUBK = 32
// floats per thread.
template <int V_SPLIT>
__global__ __launch_bounds__(128 * SUBK)
void cuda_recurrent_gdr_kernel_128_reg
(
...
)
{
    ...
    // ---- this thread's slice of the recurrent state, held for the whole seq ----
    float* gl_state = final_state + head * HEAD_STATE_SIZE;
    const bool active = (t < V_CHUNK_DIM);
    float st[BTS];
    if (active)
    {
        const float* rd = gl_state + v_start + t + (size_t) bt * BTS * HEAD_DIM;
        #pragma unroll
        for (int i = 0; i < BTS; ++i, rd += HEAD_DIM) st[i] = *rd;
    }
    ...
    for (int s = 0; s < seqlen; ++s)
    {
        ...
        if (active)
        {
            ...
            #pragma unroll
            for (int i = 0; i < BTS; ++i, ++sh_k_rd) sum = fmaf(*sh_k_rd, st[i], sum);
            atomicAdd(sh_dot1 + t, sum);
        }
        __syncthreads();
        ...
        if (active)
        {
            ...
            const float v = __bfloat162float(gl_v[t]) - sh_dot1[t] * g_h;
            float v_out = 0.0f;
            const float* sh_k_rd = sh_k + bt * BTS;
            const float* sh_q_rd = sh_q + bt * BTS;
            #pragma unroll
            for (int i = 0; i < BTS; ++i, ++sh_k_rd, ++sh_q_rd)
            {
                st[i] = st[i] * g_h + *sh_k_rd * v * beta_h;
                v_out = fmaf(*sh_q_rd, st[i], v_out);
            }
            atomicAdd(sh_dot2 + t, v_out);
        }
        __syncthreads();
    ...
    if (active)
    {
        float* wr = gl_state + v_start + t + (size_t) bt * BTS * HEAD_DIM;
        #pragma unroll
        for (int i = 0; i < BTS; ++i, wr += HEAD_DIM) *wr = st[i];
    }
}
...
)
{
    ...
    // Split of the v head dim across CTAs. The recurrence is sequential in the
    // sequence, so the ONLY parallelism is bsz * num_v_heads * v_split — with
    // v_split=4 and 48 v heads that is 192 CTAs walking 2048 dependent steps.
    // Overridable for experiments: EXL3_GDN_VSPLIT.
    // Split of the v head dim across CTAs. The recurrence is sequential in the
    // sequence, so the ONLY parallelism is bsz * num_v_heads * v_split — with 48
    // v heads that is 96 (v_split 2) to 192 (v_split 4) CTAs walking `seqlen`
    // dependent steps. 2 measured ~12% faster than 4 on Ada across seq
    // 512..4096; the trade is CTA count against per-step latency and it is not
    // monotonic (8 is ~80% slower).
    //
    // `gridDim.z` IS `v_split`, so the launched grid and the instantiated
    // V_SPLIT must agree. They previously did not: the `history` branch only
    // instantiated 4, so any other value launched CTAs that each computed the
    // whole v range and applied the state update twice — silently wrong output.
    // Only the 128/128 kernel covers every split, so anything else is clamped to
    // what its branch actually has, and the fallbacks now fail loudly.
    const bool path128 = (k_head_dim == 128 && v_head_dim == 128);
    int v_split = (bsz == 1 && k_head_dim <= 128 && v_head_dim == 128 && num_v_heads <= 64) ? 2 : 1;
    {
        const char* e = getenv("EXL3_GDN_VSPLIT");
        if (e) { int v = atoi(e); if (v == 1 || v == 2 || v == 4 || v == 8) v_split = v; }
    }
    if (!path128 && v_split != 1 && v_split != 4) v_split = 4;
    ...
    if (!history)
    {
        ...
        {
            // EXL3_GDN_NOREG=1 forces the global-memory kernel, so `attn_check gdn`
            // can verify the register-resident one against it.
            static int noreg = -1;
            if (noreg < 0) { const char* e = getenv("EXL3_GDN_NOREG"); noreg = (e && e[0] && e[0] != '0') ? 1 : 0; }
            if (noreg)
            {
                if      (v_split == 8) LAUNCH_RULE(cuda_recurrent_gated_delta_rule_kernel_128<false, 8>)
            ...
            else if (v_split == 8) LAUNCH_RULE(cuda_recurrent_gdr_kernel_128_reg<8>)
            else if (v_split == 4) LAUNCH_RULE(cuda_recurrent_gdr_kernel_128_reg<4>)
            else if (v_split == 2) LAUNCH_RULE(cuda_recurrent_gdr_kernel_128_reg<2>)
            else                   LAUNCH_RULE(cuda_recurrent_gdr_kernel_128_reg<1>)
        }
        else if (threads.x <= 128)
        {
            if      (v_split == 4) LAUNCH_RULE(cuda_recurrent_gated_delta_rule_kernel<128, false, 4>)
            else if (v_split == 1) LAUNCH_RULE(cuda_recurrent_gated_delta_rule_kernel<128, false, 1>)
            else TORCH_CHECK(false, "gdn rule: no instantiation for v_split ", v_split);
        }
    ...
    else
    {
        if (k_head_dim == 128 && v_head_dim == 128)
        {
            if      (v_split == 8) LAUNCH_RULE(cuda_recurrent_gated_delta_rule_kernel_128<true, 8>)
        ...
        else if (threads.x <= 128)
        {
            ...
            else if (v_split == 1) LAUNCH_RULE(cuda_recurrent_gated_delta_rule_kernel<128, true, 1>)
            else TORCH_CHECK(false, "gdn rule: no instantiation for v_split ", v_split);
        }
        else if (threads.x <= 256)
...
// ---------------------------------------------------------------------------
// Prefill variant of the conv1d above. That kernel is one thread per channel
// walking the sequence, which is right for a 1-token decode step but wrong for a
// 2048-token chunk: `x` is (bsz, dim, seqlen), so consecutive threads read
// addresses `seqlen * 2` bytes apart — every warp load touches 32 separate
// sectors to use 2 bytes of each — and the grid is only ceil(dim/256) CTAs.
// Measured 114 GB/s of ~1008 on an RTX 4090, which is exactly the ~1/8 the
// coalescing loss predicts.
//
// Here a CTA owns a [TD channels x TS tokens] tile. The load walks the sequence
// (contiguous in `x`) into shared memory and the store walks channels
// (contiguous in `out`), so both directions coalesce. State handling is split
// out into `conv1d_state_writeback_kernel` so this one is pure streaming.
#define CONV_TD 64
#define CONV_TS 64
// +1 makes the row stride odd in 4-byte words, so the channel-major read in the
// compute phase is shared-memory-bank-conflict free.
#define CONV_ROW (CONV_TS + CONV1D_MAX_K + 1)

template <bool ACT>
__global__ __launch_bounds__(256)
void conv1d_prefill_kernel
(
    const bfloat16* __restrict__ x,           // (bsz, dim, seqlen)
    const bfloat16* __restrict__ conv_state,  // (num_slots, dim, state_size)
    const int* __restrict__ slots,
    const bfloat16* __restrict__ weight,      // (dim, K)
...
{
    __shared__ float tile[CONV_TD][CONV_ROW];

    const int d0 = blockIdx.x * CONV_TD;
    const int s0 = blockIdx.y * CONV_TS;
    const int b  = blockIdx.z;
    const int slot = slots ? slots[b] : b;
    const int tid = threadIdx.x;

    // --- load [d0, d0+TD) x [s0-(K-1), s0+TS) ---------------------------------
    // warp `w` takes channel rows w, w+8, ...; its lanes read consecutive `s`.
    const int warp = tid / 32, lane = tid % 32;
    for (int dd = warp; dd < CONV_TD; dd += 8)
    {
        const int d = d0 + dd;
        const bool dv = (d < dim);
        const bfloat16* xd = x + ((size_t) b * dim + d) * seqlen;
        const bfloat16* st = conv_state + ((size_t) slot * dim + d) * state_size;
        for (int j = lane; j < CONV_ROW; j += 32)
        {
            const int sp = s0 + j - (K - 1);          // absolute input position
            float v = 0.f;
            if (dv && j < CONV_TS + K - 1)
            {
                if (sp >= 0)
                    v = (sp < seqlen) ? __bfloat162float(xd[sp]) : 0.f;
                else
                    v = __bfloat162float(st[K + sp]);  // state holds the last K inputs
            }
            tile[dd][j] = v;
        }
    }
    ...
    // --- compute + store: threads walk channels so `out` writes coalesce -------
    const int dd = tid % CONV_TD;
    const int sf = tid / CONV_TD;                 // 4 token rows in flight
    const int d = d0 + dd;
    if (d >= dim) return;
    float w[CONV1D_MAX_K];
    #pragma unroll
    ...
    for (int si = sf; si < CONV_TS; si += 256 / CONV_TD)
    {
        const int s = s0 + si;
        if (s >= seqlen) break;
        float acc = bias_d;
        #pragma unroll
        for (int k = 0; k < CONV1D_MAX_K; ++k)
            if (k < K) acc = fmaf(w[k], tile[dd][si + k], acc);
        if constexpr (ACT) acc *= _sigmoid_fast_exp(acc);
        out[((size_t) b * seqlen + s) * dim + d] = __float2bfloat16_rn(acc);
    }
}

// Roll `conv_state` forward past a whole chunk. Separate from the streaming
// kernel above so that one stays a pure load/store pipeline.
template <bool HISTORY>
__global__ void conv1d_state_writeback_kernel
(
...
{
    const int d = blockIdx.x * CONV1D_NUM_THREADS + threadIdx.x;
    if (d >= dim) return;
    const int b = blockIdx.y;
    const int slot = slots ? slots[b] : b;
    const bfloat16* xd = x + ((size_t) b * dim + d) * seqlen;
    bfloat16* st = conv_state + ((size_t) slot * dim + d) * state_size;

    // old_state[k] is the input at position k - K
    float old_state[CONV1D_MAX_K];
    #pragma unroll
    for (int k = 0; k < CONV1D_MAX_K; ++k)
        if (k < K) old_state[k] = __bfloat162float(st[k]);

    if constexpr (!HISTORY)
    {
        float v[CONV1D_MAX_K];
        #pragma unroll
        for (int k = 0; k < CONV1D_MAX_K; ++k)
            if (k < K)
            {
                const int src_t = seqlen + k;
                v[k] = (src_t < K) ? old_state[src_t] : __bfloat162float(xd[src_t - K]);
            }
        #pragma unroll
        for (int k = 0; k < CONV1D_MAX_K; ++k)
            if (k < K) st[k] = __float2bfloat16_rn(v[k]);
    }
    else
    {
        const int total = K + seqlen;
        const int write_size = state_size < total ? state_size : total;
        const int dst_start = state_size - write_size;
        const int src_start = total - write_size;
        for (int j = write_size - 1; j >= 0; --j)
        {
            const int src_t = src_start + j;
            const float v = (src_t < K) ? old_state[src_t] : __bfloat162float(xd[src_t - K]);
            st[dst_start + j] = __float2bfloat16_rn(v);
        }
    }
}
...
)
{
    ...
    // Long chunks take the tiled/coalesced prefill pair; short ones (decode, and
    // CUDA-graph capture, which needs the single patchable launch) keep the
    // one-thread-per-channel kernel. 64 is where the tiled version starts to win.
    const bool prefill = (seqlen >= CONV_TS && !graph);
    if (prefill)
    {
        dim3 pblocks(CEIL_DIVIDE(dim, CONV_TD), CEIL_DIVIDE(seqlen, CONV_TS), bsz);
        #define PF_ARGS                                 \
            (const bfloat16*) x.data_ptr(),             \
            (const bfloat16*) conv_state.data_ptr(),    \
            slots_ptr,                                  \
            (const bfloat16*) weight.data_ptr(),        \
            bias_ptr,                                   \
            (bfloat16*) out.data_ptr(),                 \
            dim, seqlen, state_size, K
        if (activation) conv1d_prefill_kernel<true><<<pblocks, 256, 0, stream>>>(PF_ARGS);
        else            conv1d_prefill_kernel<false><<<pblocks, 256, 0, stream>>>(PF_ARGS);
        cuda_check(cudaPeekAtLastError());
        #undef PF_ARGS
        // roll the conv window forward past this chunk
        #define WB_ARGS                                 \
            (const bfloat16*) x.data_ptr(),             \
            (bfloat16*) conv_state.data_ptr(),          \
            slots_ptr, dim, seqlen, state_size, K
        if (history) conv1d_state_writeback_kernel<true>
                        <<<blocks, CONV1D_NUM_THREADS, 0, stream>>>(WB_ARGS);
        else         conv1d_state_writeback_kernel<false>
                        <<<blocks, CONV1D_NUM_THREADS, 0, stream>>>(WB_ARGS);
        #undef WB_ARGS
    }
    else if (activation)
